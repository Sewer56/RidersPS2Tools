...
namespace RidersTextureArchiveTool
{
    public class ArchiveWriter
    {
        ...
        private void PrecomputeFileOffsets(Span<int> offsets, int stringDataSize)
        {
            ...
            int currentFileDataOffset = Utilities.Utilities.RoundUp(headerSize + fileOffsetArraySize + unknownFlagArraySize + stringDataSize, FileDataAlignment);

            // Populate offsets for every file.
            for (int x = 0; x < Files.Count; x++)
            {
                offsets[x] = currentFileDataOffset;
                currentFileDataOffset += Utilities.Utilities.RoundUp(Files[x].Data.Length, FileDataAlignment);
            }
        }
    }
}
